...
test("public asset checker blocks non-allowlisted scene bundles", async () => {
  ...
  await mkdir(join(root, "demo-scene"));
  await mkdir(join(root, "sense-private-scene"));

  const sceneDirs = await listSceneBundleDirs(root);
  assert.deepEqual(sceneDirs, ["demo-scene", "sense-private-scene"]);
  assert.deepEqual(findBlockedPublicSceneDirs(sceneDirs), ["demo-scene", "sense-private-scene"]);
  assert.deepEqual(await listSceneBundleDirs(join(root, "missing")), []);
});

...
Для участника без доступа к терминалу: [короткая инструкция по готовой встрече](try-staging.md). Организатор передаёт приватные ссылки отдельно от документации.

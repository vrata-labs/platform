...
NOAH не читает содержимое сайта, не анализирует HTML и не нажимает кнопки внутри чужой страницы. Ведущий сам управляет тем, что показывает, а остальные участники видят поток.

...
- Heavy CI runs core checks first, then executes two Playwright jobs in parallel: the serial `runtime.spec.ts` suite and all other e2e specs. The required `checks` job reports success only after core checks and both e2e jobs pass; each e2e job uploads its own report artifact.

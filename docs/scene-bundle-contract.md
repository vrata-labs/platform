...
- `spawnPoints`: array of spawn points; runtime v1 uses the first entry. A spawn point can include an optional finite `yaw` in radians.
...
- `renderProfile`: `"neutral-pbr"` enables opt-in AgX tone mapping with balanced ambient, hemisphere, and directional lighting for authored PBR scenes. Omitted bundles retain legacy rendering.

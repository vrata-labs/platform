...
Статус: Stage 0 infrastructure, Stage 1 art-direction gate, Stage 2 patched-source digest, publisher model Git/LFS pointer identity, independently observed selected TRELLIS payload byte-hash lock, DINO source/metadata identity, raw DINO payload byte-hash lock и DINO derived-safetensors/tensor-equivalence lock выполнены; generation сохраняет verdict `BLOCK` на 2026-08-20, а strict-load/runtime qualification, DINO weight rights, dependency/OCI evidence, provider evidence и human signoff остаются открытыми prerequisites.
...
| Experiment / Scene Factory repository | `vrata-labs/warm-modern-meeting-room-scene-factory`, DINO derived-runtime artifact merge `b5f11d20d6e5ec3c2bb0ef66aaea28a78dcd02c8`, post-merge CI `32418572991` green, `main` protected | Продолжать только через protected PR |
...
| Experiment storage | Private Yandex Object Storage, 10 GiB hard quota, anonymous/static-key access disabled, dedicated AES-256 SSE-KMS, lifecycle 30 days для temporary/rejected и 1 day для incomplete multipart; separate point-in-time verifications зафиксировали retention raw DINO PTH, DINO derived safetensors и четырёх selected TRELLIS payloads на 2026-08-20; это не доказательство непрерывного state; private identifiers остаются вне public Git | На момент соответствующих verifications шесть model artifacts общим размером 5099152089 bytes были retained по digest; использовать только по classification/prefix policy; model inputs по-прежнему запрещены |
...
- Для image-to-3D базовый кандидат: `microsoft/TRELLIS-image-large`; publisher Git/LFS pointer identity и четыре selected payload byte identities locked на exact revision, DINO source/raw identity и derived safetensors exact tensor-equivalence locked отдельно, но MIT/Apache model-card metadata и content hashes не заменяют weight/source rights, strict-load/runtime qualification, dependency lock или provider terms.
...
- `patchedSourceTreeDigest`, pointer-level `trellisModelArtifactLock`, `trellisModelPayloadBytesVerification`, `dinoSourceGitObjectLock`, `dinoArtifactPayloadBytesVerification` и `dinoDerivedRuntimeArtifactLock` закрыты для своих exact evidence scopes; DINO `allOf` composition `dinoSourceAndArtifactLock` закрыт механически как производная двух identity locks и не является rights-, runtime- или provider-gate; TRELLIS payload и DINO derived locks не закрывают composite gates; historical gate lists внутри content-addressed locks остаются snapshots на момент materialization, current state хранится отдельно; generation остаётся запрещённой до DINO source-versus-weight rights, patched PyTorch/dependency lock, complete OCI third-party notice bundle, OCI digest, SBOM/security report, real strict-load/offline import/inference and GPU parity tests, provider evidence и human signoff; payload bytes не добавлялись в public Git, PTH десериализовался только из sealed verified bytes с `weights_only=True`, TRELLIS safetensors не парсились, model inputs, generated outputs и compute resources не создавались;

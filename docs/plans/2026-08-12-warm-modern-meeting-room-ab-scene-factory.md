...
| Experiment / Scene Factory repository | `vrata-labs/warm-modern-meeting-room-scene-factory`, source-selection merge `a1c05322c9cc757572b7db40b75be3d23cad17ed`, post-merge CI `31843334799` green, `main` protected | Продолжать только через protected PR |
...
| Human rights owner | Public role `experiment-sponsor` назначен; dated verdict сохраняет `BLOCK`: stock TRELLIS запрещён, 53-file upstream source selection locked for patch review | Собрать immutable patched tree, dependency/model/DINO/OCI/SBOM evidence и получить human signoff |
...
- stock TRELLIS path запрещён из-за eager import code с non-commercial file-level terms и standard `to_glb`/`nvdiffrast`; upstream source-selection lock фиксирует exact TRELLIS/FlexiCubes commits, 53 source/provenance/license files, selection digest `5860f91b0fddd401f661f5a16ef2f224d3c6f712f73a2fb050fd547abcac8348` и complete-policy digest `d810ca9db4ac72dc53473fc59ce2fb35996bf7cf48e7c2a2246b4bfbf733e5ea`;
- source selection не является patched runtime или rights approval; generation остаётся запрещённой до patched-tree digest, patched PyTorch/dependency lock, DINO/model artifact locks, third-party notice bundle, OCI digest, SBOM/security report, offline/GPU parity и human signoff;

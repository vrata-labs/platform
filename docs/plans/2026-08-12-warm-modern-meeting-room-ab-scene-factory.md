...
Статус: Stage 0 infrastructure, Stage 1 art-direction gate и Stage 2 patched-source digest выполнены, Definition of Ready частично открыт; AI probe launch остаётся заблокирован на 2026-08-15.
...
| Experiment / Scene Factory repository | `vrata-labs/warm-modern-meeting-room-scene-factory`, patched-source merge `9a22a34f17b7e0f073ba1684389c75fe1cfc6d90`, post-merge CI `31852913450` green, `main` protected | Продолжать только через protected PR |
...
| Human rights owner | Public role `experiment-sponsor` назначен; dated verdict сохраняет `BLOCK`: stock TRELLIS запрещён, 50-file patched source artifact materialized and static-verified, runtime/model/provider gates open | Собрать dependency/model/DINO/OCI/SBOM/runtime evidence и получить human signoff |
...
- `patchedSourceTreeDigest` закрыт только для exact artifact; generation остаётся запрещённой до patched PyTorch/dependency lock, DINO/model artifact locks, complete OCI third-party notice bundle, OCI digest, SBOM/security report, real offline import/inference and GPU parity tests, provider evidence и human signoff; weights, model inputs, generated outputs, containers и cloud resources не добавлялись;

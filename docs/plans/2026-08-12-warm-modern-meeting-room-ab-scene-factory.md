...
Статус: Stage 0 infrastructure, Stage 1 art-direction gate, Stage 2 patched-source digest и publisher model Git/LFS pointer identity выполнены, Definition of Ready частично открыт; AI probe launch остаётся заблокирован на 2026-08-15.
...
| Experiment / Scene Factory repository | `vrata-labs/warm-modern-meeting-room-scene-factory`, model-artifact merge `408ba28db9447a950ad403e1d2e00c05bafc621d`, post-merge CI `31871464748` green, `main` protected | Продолжать только через protected PR |
...
| Human rights owner | Public role `experiment-sponsor` назначен; dated verdict сохраняет `BLOCK`: stock TRELLIS запрещён, 50-file patched source artifact materialized and static-verified, publisher commit/tree/19 blobs/8 LFS pointers TRELLIS-image-large locked, payload/license/runtime/provider gates open | Независимо получить и проверить bytes четырёх selected payloads, собрать dependency/DINO/OCI/SBOM/runtime evidence и получить human signoff |
...
- Для image-to-3D базовый кандидат: `microsoft/TRELLIS-image-large`; publisher Git/LFS pointer identity locked на exact revision, но MIT model-card metadata без standalone weight license и без проверки payload bytes не заменяет проверку weights, dependencies и provider terms.
...
- `patchedSourceTreeDigest` и pointer-level `trellisModelArtifactLock` закрыты только для exact artifacts; historical gate list внутри content-addressed patched-source lock остаётся snapshot на момент materialization, current state хранится отдельно; generation остаётся запрещённой до `trellisModelPayloadBytesVerification`, patched PyTorch/dependency lock, DINO lock, complete OCI third-party notice bundle, OCI digest, SBOM/security report, real offline import/inference and GPU parity tests, provider evidence и human signoff; payload bytes, model inputs, generated outputs, containers и cloud resources не добавлялись;

...
Статус: Stage 0 infrastructure, Stage 1 art-direction gate, Stage 2 patched-source digest, publisher model Git/LFS pointer identity, DINO source/metadata identity и independently observed raw DINO payload byte-hash lock выполнены; generation сохраняет verdict `BLOCK` на 2026-08-20, а derived-runtime qualification, DINO weight rights, provider evidence и human signoff остаются открытыми prerequisites.
...
| Experiment / Scene Factory repository | `vrata-labs/warm-modern-meeting-room-scene-factory`, DINO raw payload identity merge `0d421a2450016f954f1d7aea20a50ba1aed42e9f`, post-merge CI `32357339985` green, `main` protected | Продолжать только через protected PR |
...
| Human rights owner | Public role `experiment-sponsor` назначен; dated verdict сохраняет `BLOCK`: stock TRELLIS запрещён, 50-file patched source artifact materialized and static-verified, TRELLIS publisher commit/tree/19 blobs/8 LFS pointers locked, DINO source/content/object graph locked, raw publisher response bytes independently hashed and content-address locked; publisher SHA-256 отсутствует, DINO rights/derived-runtime и остальные non-identity gates open | Независимо получить и проверить bytes четырёх selected TRELLIS payloads, отдельно lock любой derived DINO runtime artifact, собрать DINO source-versus-weight rights, dependency/OCI/SBOM/runtime evidence и получить human signoff |
...
| Experiment storage | Private Yandex Object Storage, 10 GiB hard quota, anonymous/static-key access disabled, dedicated AES-256 SSE-KMS, lifecycle 30 days для temporary/rejected и 1 day для incomplete multipart; general controls являются point-in-time operator evidence на 2026-08-14, а отдельная payload verification зафиксировала retention raw DINO PTH на 2026-08-20; это не доказательство непрерывного state; private identifiers остаются вне public Git | На момент payload verification raw DINO PTH размером 1217607321 bytes был retained по digest; использовать только по classification/prefix policy; model inputs по-прежнему запрещены |
...
- Для image-to-3D базовый кандидат: `microsoft/TRELLIS-image-large`; publisher Git/LFS pointer identity locked на exact revision, DINO source identity и raw publisher PTH byte identity locked отдельно, но MIT/Apache model-card metadata и independent DINO byte hash не заменяют TRELLIS payload verification, DINO source-versus-weight rights, derived-runtime qualification, dependency lock и provider terms.
...
- [ ] [1-2ч] Получить четыре selected TRELLIS payloads напрямую в restricted storage, независимо проверить SHA-256/size против publisher pointers и сохранить по digest.
...
- `patchedSourceTreeDigest`, pointer-level `trellisModelArtifactLock`, `dinoSourceGitObjectLock` и `dinoArtifactPayloadBytesVerification` закрыты только для exact identity evidence; DINO `allOf` composition `dinoSourceAndArtifactLock` закрыт механически как производная двух identity locks и не является rights-, derived-runtime- или provider-gate; historical gate lists внутри content-addressed locks остаются snapshots на момент materialization, current state хранится отдельно; generation остаётся запрещённой до `trellisModelPayloadBytesVerification`, `dinoDerivedRuntimeArtifactLock`, DINO source-versus-weight rights, patched PyTorch/dependency lock, complete OCI third-party notice bundle, OCI digest, SBOM/security report, real offline import/inference and GPU parity tests, provider evidence и human signoff; payload bytes не добавлялись в public Git, PTH не десериализовался, model inputs, generated outputs, containers и compute resources не создавались;

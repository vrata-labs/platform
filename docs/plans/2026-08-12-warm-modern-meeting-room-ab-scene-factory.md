...
Статус: Stage 0 infrastructure, Stage 1 art-direction gate, Stage 2 patched-source digest, publisher model Git/LFS pointer identity, independently observed selected TRELLIS payload byte-hash lock, DINO source/metadata identity и raw DINO payload byte-hash lock выполнены; generation сохраняет verdict `BLOCK` на 2026-08-20, а runtime qualification, DINO weight rights, dependency/OCI evidence, provider evidence и human signoff остаются открытыми prerequisites.
...
| Зависимость | Состояние на 2026-08-20 | Следующее действие |
|---|---|---|
...
| Experiment / Scene Factory repository | `vrata-labs/warm-modern-meeting-room-scene-factory`, selected TRELLIS payload identity merge `d83bcb76d6022d4ed705788dd482e48f44c0cbb2`, post-merge CI `32375380842` green, `main` protected | Продолжать только через protected PR |
...
| GPU provider/budget | Yandex Cloud Compute; proposed preemptible `standard-v3-t4i`, 24 GB VRAM, `ru-central1-d`, 32.86192 RUB/hour all-in floor, 120-minute first-run maximum 65.72384 RUB, proposed campaign cap 1,000 RUB; billing/teardown owner role assigned; fail-closed provider janitor implementation merged | Получить явное budget/launch approval, quota, deploy janitor и пройти no-GPU provider fixture; GPU VM не создавать раньше всех gates |
| Human rights owner | Public role `experiment-sponsor` назначен; dated verdict сохраняет `BLOCK`: stock TRELLIS запрещён, 50-file patched source artifact materialized and static-verified, TRELLIS publisher commit/tree/19 blobs/8 LFS pointers locked, четыре selected TRELLIS payload bytes independently match publisher pointer SHA-256/size, DINO source/content/object graph и independently observed raw PTH bytes locked; DINO rights/derived-runtime и остальные non-identity gates open | Отдельно lock любой derived DINO runtime artifact, собрать DINO source-versus-weight rights, dependency/OCI/SBOM/runtime evidence и получить human signoff |
...
| Experiment storage | Private Yandex Object Storage, 10 GiB hard quota, anonymous/static-key access disabled, dedicated AES-256 SSE-KMS, lifecycle 30 days для temporary/rejected и 1 day для incomplete multipart; general controls являются point-in-time operator evidence на 2026-08-14, а отдельные payload verifications зафиксировали retention raw DINO PTH и четырёх selected TRELLIS payloads на 2026-08-20; это не доказательство непрерывного state; private identifiers остаются вне public Git | На момент соответствующих verifications пять raw payloads общим размером 3881628681 bytes были retained по digest; использовать только по classification/prefix policy; model inputs по-прежнему запрещены |
...
- Для image-to-3D базовый кандидат: `microsoft/TRELLIS-image-large`; publisher Git/LFS pointer identity и четыре selected payload byte identities locked на exact revision, DINO source identity и raw publisher PTH byte identity locked отдельно, но MIT/Apache model-card metadata и independent byte hashes не заменяют weight/source rights, derived-runtime qualification, dependency lock, provider terms или runtime validation.
...
- [ ] [1-2ч] Если runtime использует производный safetensors, зафиксировать exact converter/options, tensor manifest, size/SHA-256 и tensor-equivalence evidence до offline runtime qualification.
...
- `patchedSourceTreeDigest`, pointer-level `trellisModelArtifactLock`, `trellisModelPayloadBytesVerification`, `dinoSourceGitObjectLock` и `dinoArtifactPayloadBytesVerification` закрыты только для exact identity evidence; DINO `allOf` composition `dinoSourceAndArtifactLock` закрыт механически как производная двух identity locks и не является rights-, derived-runtime- или provider-gate; TRELLIS payload lock не закрывает composite gate; historical gate lists внутри content-addressed locks остаются snapshots на момент materialization, current state хранится отдельно; generation остаётся запрещённой до `dinoDerivedRuntimeArtifactLock`, DINO source-versus-weight rights, patched PyTorch/dependency lock, complete OCI third-party notice bundle, OCI digest, SBOM/security report, real offline import/inference and GPU parity tests, provider evidence и human signoff; payload bytes не добавлялись в public Git, PTH не десериализовался, safetensors не парсились, model inputs, generated outputs, containers и compute resources не создавались;
- primary compute proposal: preemptible T4i 24 GB VRAM, 100 GB auto-delete SSD и dynamic IPv4, 32.86192 RUB/hour, 120-minute maximum 65.72384 RUB, proposed campaign cap 1,000 RUB; fallback A100 требует отдельного approval;

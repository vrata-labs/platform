...
Статус: Stage 0 infrastructure, Stage 1 art-direction gate, Stage 2 patched-source digest, publisher model Git/LFS pointer identity и DINO source/metadata identity выполнены, Definition of Ready частично открыт; AI probe launch остаётся заблокирован на 2026-08-15.
...
| Зависимость | Состояние на 2026-08-15 | Следующее действие |
|---|---|---|
...
| Experiment / Scene Factory repository | `vrata-labs/warm-modern-meeting-room-scene-factory`, DINO source/metadata merge `5db0ade717e763e7c91b91860c1026e97d619b20`, post-merge CI `31878574176` green, `main` protected | Продолжать только через protected PR |
...
| Human rights owner | Public role `experiment-sponsor` назначен; dated verdict сохраняет `BLOCK`: stock TRELLIS запрещён, 50-file patched source artifact materialized and static-verified, TRELLIS publisher commit/tree/19 blobs/8 LFS pointers locked, DINO source/content/object graph locked и один publisher HEAD observation записан, payload/license/runtime/provider gates open | Независимо получить и проверить bytes четырёх selected TRELLIS payloads и raw DINO PTH, отдельно lock любой derived DINO runtime artifact, собрать dependency/OCI/SBOM/runtime evidence и получить human signoff |
...
- Для image-to-3D базовый кандидат: `microsoft/TRELLIS-image-large`; publisher Git/LFS pointer identity locked на exact revision, DINO source identity locked отдельно, но MIT/Apache model-card metadata, один DINO HEAD observation и отсутствие самостоятельно проверенных payload bytes не заменяют проверку weights, dependencies и provider terms.
...
- [ ] [1-2ч] Получить raw DINO PTH напрямую в restricted storage, повторно проверить zero-redirect metadata continuity, независимо вычислить SHA-256/size и сохранить по digest.
...
- один zero-redirect DINO publisher HEAD observation записал content length 1217607321, opaque version ID `HLmbhvcd2hPq9CNLwMvwswbRlzZRuOeA` и multipart ETag без GET/range fallback или доставки response-body bytes verifier code; publisher/observed SHA-256 остаются null, raw PTH не скачан, а broad research-only и неполные ChannelAdaptive README statements остаются unresolved human-review caveat;
...
- `patchedSourceTreeDigest`, pointer-level `trellisModelArtifactLock` и `dinoSourceGitObjectLock` закрыты только для exact identity evidence; historical gate lists внутри content-addressed locks остаются snapshots на момент materialization, current state хранится отдельно и вычисляет DINO `allOf` composition по members; generation остаётся запрещённой до `trellisModelPayloadBytesVerification`, `dinoArtifactPayloadBytesVerification`, `dinoDerivedRuntimeArtifactLock`, patched PyTorch/dependency lock, complete OCI third-party notice bundle, OCI digest, SBOM/security report, real offline import/inference and GPU parity tests, provider evidence и human signoff; payload bytes, model inputs, generated outputs, containers и cloud resources не добавлялись;

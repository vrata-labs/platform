...
Статус: Stage 0 infrastructure, Stage 1 art-direction gate и Stage 2 runtime qualification выполнены; первый internal pruned TRELLIS chair probe прошёл на Tesla T4 с project-authored input, measured VRAM, content-addressed output и полным teardown. Общий AI feasibility gate остаётся открытым до второго успешного component probe; production publication по-прежнему требует DINO/model rights, OCI/SBOM/notices, provider snapshot и отдельный publication signoff.
...
| Experiment / Scene Factory repository | `vrata-labs/warm-modern-meeting-room-scene-factory`, GPU probe merge `ee626ef08bc23d3e3d990a30b444490138992ada`, feasibility-gate correction `e28d8b620e60f1dceb767a69b778c5f3d7ce395e`, post-merge CI `32654875876` green, `main` protected | Продолжать только через protected PR |
...
| GPU provider/budget | Yandex Cloud Compute; quota `compute.instanceT4Gpus.count=1`, successful preemptible `standard-v3-t4` probe в `ru-central1-a`, reported GPU memory 15655829504 bytes, guest watchdog 45 minutes; three disposable VM attempts, zero probe resources after teardown | Сверить фактическое provider billing до следующего paid run и заново явно arm deletion guard |
...
| Experiment storage | Private Yandex Object Storage, 10 GiB hard quota, anonymous/static-key access disabled, dedicated AES-256 SSE-KMS и lifecycle controls; model payloads, one project-authored input, raw mesh, report, optimized GLB и preview прошли point-in-time full readback by SHA-256; private identifiers остаются вне public Git | Сохранить accepted raw output по content address; удалить временные локальные model copies после завершения evidence work |
...
- `patchedSourceTreeDigest`, exact model/payload/dependency locks, patched PyTorch, offline strict loads, `gpuParityAndVramTest` и internal probe signoff закрыты для exact evidence scopes; internal pruned generation разрешена только для project-authored inputs, а production publication gates остаются открытыми;
...
- Yandex quota `compute.instanceT4Gpus.count=1` получена по support ticket `FS946793`; успешный preemptible T4 run выполнен в `ru-central1-a`, фактическое provider billing остаётся pending reconciliation;
...
- три disposable GPU VM attempts и их auto-delete disks удалены; temporary service account и access bindings удалены, postflight показал zero probe resources. Вместо первоначально требовавшегося provider-side function janitor использовались guest watchdog и заранее armed local deletion timer; это explicit one-probe exception, не production precedent.

AI feasibility gate:

- chair probe является первым успешным из требуемых минимум двух component probes; Stage 3 остаётся заблокированным до второго pass;

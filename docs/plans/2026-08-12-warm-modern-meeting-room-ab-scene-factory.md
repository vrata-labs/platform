...
Статус: Stage 0 infrastructure, Stage 1 art-direction gate, Stage 2 revised patched-source digest, publisher model Git/LFS pointer identity, selected TRELLIS и DINO payload identities, DINO derived-safetensors/tensor-equivalence lock, exact dependency wheel/runtime-site lock, patched PyTorch qualification и offline imports/strict loads выполнены; generation сохраняет verdict `BLOCK` на 2026-08-23, а DINO weight rights, OCI/SBOM/notices, provider evidence, GPU parity/VRAM и human signoff остаются открытыми prerequisites.
...
| Experiment / Scene Factory repository | `vrata-labs/warm-modern-meeting-room-scene-factory`, runtime qualification merge `92b01e08bcc3fdc5061b02371b5414d2420b0c6b`, post-merge CI `32634437845` green, `main` protected | Продолжать только через protected PR |
...
| Human rights owner | Public role `experiment-sponsor` назначен; dated verdict сохраняет `BLOCK`: revised 50-file patched source, exact 41-wheel/runtime-site environment, patched PyTorch security probe, 58 source imports с verified origins и пять strict loads locked; DINO rights, OCI/SBOM/notices, provider/GPU и human-signoff gates open | Собрать DINO source-versus-weight rights, OCI/SBOM/notices и provider/GPU evidence, затем получить human signoff |
...
- `patchedSourceTreeDigest`, `trellisModelArtifactLock`, `trellisModelPayloadBytesVerification`, `dinoSourceGitObjectLock`, `dinoArtifactPayloadBytesVerification`, `dinoDerivedRuntimeArtifactLock`, `dependencyWheelHashLock`, `patchedPytorchQualification` и `offlineImportRuntimeTest` закрыты для exact evidence scopes; DINO `allOf` composition `dinoSourceAndArtifactLock` закрыт механически и не является rights- или provider-gate; historical gate lists остаются snapshots, current state хранится отдельно; generation остаётся запрещённой до DINO source-versus-weight rights, complete OCI notices/digest, SBOM/security report, inference/GPU parity and VRAM tests, provider evidence и human signoff; payload bytes не добавлялись в public Git, model inputs, generated outputs и compute resources не создавались;

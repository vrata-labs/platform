...
function runFrame(state: FlowState, input: {
  ...
  executeCommands(state, movementPlan.commands);

  if (movementPlan.kind === "seat_lock") {
    state.pose = {
      ...state.pose,
      position: movementPlan.position,
      yaw: movementPlan.yaw ?? state.pose.yaw
    };
    state.lastAppliedSeatLockId = movementPlan.seatId;
    return;
  }

  if (movementPlan.debugLocomotionMode) {
    state.debugLocomotionMode = movementPlan.debugLocomotionMode;
  }
  if (movementPlan.movementReason) {
    state.pose = movementPlan.pose;
  }
}

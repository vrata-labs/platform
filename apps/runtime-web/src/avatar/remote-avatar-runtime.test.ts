...
test("remote avatar runtime keeps desktop strafe body yaw stable from pose root", () => {
  const scene = new THREE.Scene();
  const runtime = createRemoteAvatarRuntime({
    scene,
    bodyGeometry: new THREE.CapsuleGeometry(0.24, 0.8, 6, 12),
  ...
  runtime.ingestPoseFrame("remote-strafe", {
    seq: 1,
    sentAtMs: Date.now() - 100,
    flags: 0,
    root: { x: 0, y: 0, z: 0, yaw: 0, vx: 0, vz: 0.85 },
    head: { x: 0.15, y: 1.6, z: 0, qx: 0, qy: 0, qz: 0, qw: 1 },
    leftHand: { x: -0.2, y: 1.2, z: 0, qx: 0, qy: 0, qz: 0, qw: 1, gesture: 1 },
    rightHand: { x: 0.2, y: 1.2, z: 0, qx: 0, qy: 0, qz: 0, qw: 1, gesture: 1 },
    locomotion: { mode: 2, speed: 0.85, angularVelocity: 0 }
  }, debugState);
  runtime.ingestPoseFrame("remote-strafe", {
    seq: 2,
    sentAtMs: Date.now(),
    flags: 0,
    root: { x: 0.4, y: 0, z: 0, yaw: 0, vx: 0, vz: 0.85 },
    head: { x: 0.55, y: 1.6, z: 0, qx: 0, qy: 0, qz: 0, qw: 1 },
    leftHand: { x: 0.2, y: 1.2, z: 0, qx: 0, qy: 0, qz: 0, qw: 1, gesture: 1 },
    rightHand: { x: 0.6, y: 1.2, z: 0, qx: 0, qy: 0, qz: 0, qw: 1, gesture: 1 },
    locomotion: { mode: 2, speed: 0.85, angularVelocity: 0 }
  }, debugState);
  ...
  const body = scene.children.find((item) => item instanceof THREE.Mesh) as THREE.Mesh | undefined;
  assert.ok(body);
  assert.equal(Math.abs(body.rotation.y) < 0.2, true);
});

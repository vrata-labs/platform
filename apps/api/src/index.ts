...
async function handleRequest(request: IncomingMessage, response: ServerResponse): Promise<void> {
  ...
  if (method === "GET" && (url.pathname === "/compatibility" || url.pathname === "/compatibility.html")) {
    const served = await serveStatic(response, join(runtimeStaticRoot, "compatibility.html"));
    if (!served) json(response, 503, { error: "compatibility_build_missing" });
    return;
  }

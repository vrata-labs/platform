...
- Phase 2 avatar sync is now closed on staging: reliable state relay/replay, transient pose relay, reconnect/late join, same-browser tab identity separation, adaptive playback delay, scene-room avatar boot ordering, secure selector links, and default-on avatars were all shipped and verified.
- `participantId` must stay per-tab in `sessionStorage`, not `localStorage`; otherwise multiple tabs in one browser collapse into one remote participant and users stop seeing each other.
- In heavy scene rooms like `Hall`, boot `room-state` and the local avatar session before long scene-bundle loading; delaying avatar boot until after scene load can make remote avatars appear missing even though presence still works.
- Movement should follow the current camera forward vector for both keyboard and VR stick input; using only stored `yaw` can drift after scene transitions or XR entry and makes "forward" move in the wrong direction.
- For remote VR observers on web, do not rely only on hand `gesture > 0` to keep hands visible; `vr-controller` / `vr-hand` participants need a stickier hand-visibility path or their hands appear to disappear despite valid pose frames.
- Staging smoke for avatars should cover not only `demo-room` but also heavy scene rooms like `Hall`, plus same-browser tab identity, selector transitions, and remote hand visibility; otherwise regressions can hide behind simpler rooms.
